...
import {fetchForm} from './fetch.mjs';
...
function inputCheck($input) {
    ...
    const telRegexp = /^(\+7|8)[\s(]{0,3}[\d]{3,4}[\s)]{0,3}([\s-]{0,3}[\d]{2,3}){3}$/;
    const emailRegexp = /^[A-Za-z0-9]+[\w.-]*[A-Za-z0-9]+@[\w-]+\.[A-Za-z]{2,5}$/i;
    ...
    const telCheck = () => {
        if (
            ...
            error = 'Поле должно содержать валидный номер телефона';
            return true;
        }
        if (
            $input.type === 'tel'
            && $input.replaceAll(/[\s()+-]/g, '').length !== 11
        ) {
            error = 'Поле должно содержать 11 цифр номера телефона';
            return true;
        }
    };
    const lengthCheck = () => {
